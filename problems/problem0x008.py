...
# not elegant, should read from input file
n = [
...
# calculate the product of adjacent digits given a starting index
def prod_adj_digits (array, index, number_of_digits):
    product = 1
    for factor in range (index, number_of_digits + index):
        product = product * array[factor]
        if product == 0:
            return (product)
    return (product)

#assert prod_adj_digits (n, 0, number_adj_digits) == 0
#assert prod_adj_digits (n, 2, number_adj_digits) == 0
#assert prod_adj_digits (n, 3, number_adj_digits) == 882


product_array = []

# given array of digits, make array of products of their adjacent members
def make_products_array (array):
    last_prod = len(array) - number_adj_digits + 1
    for index in range (0, last_prod):
        product_array.append(prod_adj_digits (n, index, number_adj_digits))

make_products_array (n)
print (max(product_array))
